...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--output-dir', default='outputs/qwena1/standalone_infer')
...
def main() -> None:
    ...
    with torch.no_grad():
        for index in indices:
            ...
            pred = run_standalone_sample_actions(
                policy,
                batch_inputs,
                make_shared_noise(
                    args.seed,
    ...
    if args.num_episodes > 0:
        eval_summary = run_open_loop_evaluation(
            mode='standalone',
            policy=policy,
            config=config,
            dataset=dataset,
            ...
            run_sample_actions=run_standalone_sample_actions,
